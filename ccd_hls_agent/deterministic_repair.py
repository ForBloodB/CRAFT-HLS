...
def _has_dynamic_loop_bound(kernel: Path) -> bool:
    text = read_text(kernel)
    return bool(re.search(r"for\s*\([^;]+;\s*[^;]+<\s*[A-Za-z_][A-Za-z0-9_]*\s*;", text))
...
def apply_deterministic_repair(
    *,
    ...
    requested_actions = action_ids or action_ids_for_failure_class(failure_class)

    if ACTION_ADD_MISSING_INCLUDE in requested_actions and _ensure_include(kernel, header):
    ...
    if ACTION_ALIGN_SIGNATURE in requested_actions:
        if top and signature and _repair_signature_name(kernel, signature, top):
            changed.append(str(kernel))
            messages.append(f"renamed generated top function to {top}")
        elif not top or not signature:
            not_applicable.append("ALIGN_SIGNATURE missing top.txt or header signature")

    if ACTION_REWRITE_INDEX_ORDER in requested_actions or ACTION_FIX_ARRAY_SHAPE in requested_actions:
        if _repair_aes_state_column_misindex(kernel, capsule):
            changed.append(str(kernel))
            messages.append("rewrote AES-style state[i][col] indexing to state[col][i]")
        else:
            not_applicable.append("FIX_ARRAY_SHAPE/REWRITE_INDEX_ORDER found no supported local index rewrite")

    if ACTION_FIX_DATA_FILE_PATH in requested_actions:
        if build_dir is not None:
            ...
            if copied:
                ...
                not_applicable.append("FIX_DATA_FILE_PATH found no runtime data files to stage")
        else:
            not_applicable.append("FIX_DATA_FILE_PATH requires a build directory")

    if ACTION_FIX_STATIC_LOOP_BOUND in requested_actions:
        if _has_dynamic_loop_bound(kernel):
            not_applicable.append("FIX_STATIC_LOOP_BOUND detected dynamic loop bounds but v1 has no safe deterministic rewrite")
        else:
            not_applicable.append("FIX_STATIC_LOOP_BOUND found no obvious dynamic loop-bound pattern")

    applied = bool(changed)
    return DeterministicRepairResult(
        applied=applied,
        repair_type=",".join(requested_actions) if requested_actions else failure_class or "preflight",
        message="; ".join(messages) if messages else "No deterministic repair applied.",
        changed_files=changed,
        not_applicable_reason="; ".join(not_applicable) if not applied and not_applicable else None,
    )
